...
class ButterFly {
    constructor(width, height) {
        ...
        this.drawButterFly = (xy1, xy2, radius) => {
            let data = this.setButterFly(radius);
            let butterfly = new Polygon(gl, data);
            let butterfly_fill_color = gl.createLinearGradient(xy1.location_x, xy1.location_y, xy2.location_x, xy2.location_y);
...
const background = (width, height) => {
    ...
    let back_color = gl.createLinearGradient(width2, 0, width2, height);
...
const renderButterFly = (width, height) => {
